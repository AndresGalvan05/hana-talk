...
| 2026-08-06 | Grafana dashboards for `ai-exercise-svc`/`event-worker`, closing the "dashboards are a separate future step" cut line from `service-metrics-export`. Two new checked-in JSON files (`infra/grafana/ai-exercise-svc-overview.json`, `event-worker-overview.json`), manually-imported like the existing `core-api-overview.json` — no API-provisioned dashboards, no new deploy mechanism. Implemented directly rather than through a new OpenSpec change, matching the precedent that the original core-api dashboard was itself just one task inside a broader change. Every panel query was run against real Grafana Cloud data via Explore before being committed, which surfaced a genuine fact worth documenting rather than assuming: the two services emit different OTel HTTP semantic-convention versions — `ai-exercise-svc`'s `FastAPIInstrumentor` uses the older convention (`http_server_duration_milliseconds_*`, labeled `http_target`/`http_method`/`http_status_code`), `event-worker`'s `otelhttp` uses the newer stable one (`http_server_request_duration_seconds_*`, labeled `http_route`/`http_request_method`/`http_response_status_code`). The two dashboards' HTTP panels are intentionally not identical as a result. |
| 2026-08-06 | Three Grafana Cloud alert rules added (`ai-exercise-svc: 5xx error rate`, `event-worker: 5xx error rate`, `event-worker: Kafka processing failures`), closing the alerting half of the observability work. Created directly in the Grafana UI, not as checked-in JSON — unlike dashboards, alert rules don't have the same lightweight paste-and-import flow, so this stays UI-only state for now. All three route to a contact point literally named `empty` (`<empty contact point>`, no email/Slack/webhook wired) — deliberate, since a real notification destination wasn't asked for and isn't needed to make the rules functional/demoable. Implemented directly, same reasoning as the dashboards. |
