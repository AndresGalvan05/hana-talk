...
core-api's metrics and traces are exportable to an external, authenticated
OTLP-compatible backend (Grafana Cloud in practice), independent of and
without disrupting the local, unauthenticated Jaeger-based dev/test setup.
No in-cluster collector, agent, or Prometheus/Grafana deployment is
required — export is direct from core-api.
