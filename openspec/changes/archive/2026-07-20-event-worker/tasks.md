...
- [x] 1.1 Install a Go toolchain on this machine if not already present
      (none installed as of this proposal)
- [x] 1.2 `event-worker/go.mod`, `cmd/event-worker/main.go` skeleton
- [x] 1.3 Add `github.com/segmentio/kafka-go` and `github.com/jackc/pgx/v5`
      dependencies
- [x] 1.4 Config via env vars: `KAFKA_BOOTSTRAP_SERVERS`, `DB_URL`
      (or discrete host/user/password/dbname), `INTERNAL_API_PORT`

## 2. Schema and migrations

- [x] 2.1 `migrations/` directory (embedded via `embed`), applied via
      `golang-migrate` on startup, targeting a dedicated `event_worker`
      Postgres schema (not `public`)
- [x] 2.2 Migration: `users(user_id UUID PRIMARY KEY, username TEXT)`
- [x] 2.3 Migration: `daily_activity(user_id UUID, activity_date DATE,
      PRIMARY KEY (user_id, activity_date))`
- [x] 2.4 Migration: `user_streaks(user_id UUID PRIMARY KEY,
      current_streak INT NOT NULL DEFAULT 0, last_active_date DATE)`

## 3. Kafka consumer

- [x] 3.1 Consumer group subscribed to `user.registered` and
...
- [x] 3.2 `user.registered` handler: upsert into `users` (`ON CONFLICT
      (user_id) DO UPDATE SET username = EXCLUDED.username`)
- [x] 3.3 `exercise.completed` handler: derive UTC date from
      ...
      → reset to 1)
- [x] 3.4 Commit consumer offsets only after the DB write succeeds

## 4. Internal read API

- [x] 4.1 `GET /leaderboard` (top N by `current_streak` desc, joined
      against the local `users` projection for display names) using Go
      1.22+ `net/http` `ServeMux` method+path routing, no router
      dependency
- [x] 4.2 `GET /users/{userId}/streak` — returns `{current_streak: 0,
      last_active_date: null}`-shaped response for a user with no
      activity yet, not an error
- [x] 4.3 Tests: idempotent upsert (`user.registered` consumed twice
...
- [x] 5.1 New `EventWorkerClient` (mirrors `AiExerciseSvcClient`'s
      `RestClient`-with-timeout shape) with a base URL from
      `EVENT_WORKER_URL`
- [x] 5.2 `UserProfileController` gains `GET /me/streak`, delegating to
      `EventWorkerClient`
- [x] 5.3 New `LeaderboardController` for `GET /api/leaderboard`,
      delegating to `EventWorkerClient`
- [x] 5.4 Tests (`@WebMvcTest`, matching existing controller test
...
- [x] 7.1 `docker compose up -d --build`: register a fresh user, complete
      ...
      shows a streak of 1
- [x] 7.2 Confirm `GET /api/leaderboard` includes that user with the
      correct username (sourced from `event-worker`'s own `users`
      projection, not core-api's database)
- [x] 7.3 Restart the `event-worker` container mid-flow (simulating a
      consumer crash) and confirm no duplicate/incorrect streak state
      after it rejoins the consumer group and reprocesses
- [x] 7.4 Confirm a user with no completions yet gets a 0 streak, not an
      error, from `GET /api/users/me/streak`
- [x] 7.5 Update `docs/ROADMAP.md` (M4 done), `docs/DEVLOG.md` (session
