...
- [ ] 1.1 Add `io.micrometer:micrometer-registry-otlp` to
      `build.gradle.kts`
- [ ] 1.2 `application.yml`: add `management.otlp.metrics.export.url`,
      `.enabled` (default `false`), `.headers.Authorization` (default
      empty)
- [ ] 1.3 `application.yml`: add `management.otlp.tracing.headers.Authorization`
      (default empty) alongside the existing `management.otlp.tracing.endpoint`
- [ ] 1.4 `sh gradlew ktlintCheck test bootJar` green

## 2. Local verification wiring

- [ ] 2.1 Add `GRAFANA_CLOUD_ENV_PATH` support to
...
- [ ] 2.2 Document the expected shape (`OTEL_EXPORTER_OTLP_ENDPOINT`,
...
- [ ] 3.1 `infra/grafana/core-api-overview.json`: a Grafana dashboard
...
- [ ] 4.1 Ask the user to create `~/.config/dev-projects/grafana-cloud.env`
...
- [ ] 4.2 `docker compose up -d --build` with `GRAFANA_CLOUD_ENV_PATH` set;
...
- [ ] 4.3 Import `core-api-overview.json` into Grafana Cloud and confirm
      its panels render real data

## 5. Production rollout (documented, executed by the user against the live cluster)

- [ ] 5.1 Document in `infra/k8s/README.md`: add the OTLP metrics URL to
...
- [ ] 6.1 Update `docs/ROADMAP.md` (this M5 step done) and `docs/DEVLOG.md`
      (session entry)
