...
- [ ] 1.1 Add `Exercise` and `AttemptResult` interfaces to
...
- [ ] 1.2 Confirm `api.get<Exercise[]>` / `api.post<AttemptResult>` work
...
- [ ] 2.1 Create `frontend/src/components/ExercisePractice.tsx` (or
      `pages/`-local, matching existing file organization) accepting
      `lessonId: string` and `onCompleted: () => void`
- [ ] 2.2 Fetch exercises on mount; loading state starts as "Loading
      exercises…" and upgrades to a "first-time generation" message via a
      ~4s `setTimeout` if the fetch hasn't resolved yet
- [ ] 2.3 Render each exercise as a card: MCQ as a radio-button group over
      `options`, fill-in-blank as a text input; each card holds its own
      `answer`/`submitting`/`result: boolean | null` state
- [ ] 2.4 Submit handler POSTs the attempt, sets that card's `result`, and
      — only when `result === true` — calls `onCompleted()`
- [ ] 2.5 Incorrect result allows immediate resubmission (don't disable
      the input/button after an incorrect attempt)
- [ ] 2.6 Fetch failure renders an error message with a retry button that
      re-runs the fetch

## 3. `LessonPage` integration

- [ ] 3.1 Render `<ExercisePractice lessonId={lessonId} onCompleted={...}
      />` below the existing lesson content/manual-complete section
- [ ] 3.2 `onCompleted` reuses the exact same state transition
...
- [ ] 4.1 Add exercise-card classes to `frontend/src/index.css` following
      the existing plain-CSS, semantic-class-name convention (no CSS
      modules, no new dependency)

## 5. Verification

- [ ] 5.1 `docker compose up -d --build` + `npm run dev`; via the
...
- [ ] 5.2 Submit an incorrect answer, confirm feedback shows and
      ...
      page reload
- [ ] 5.3 Reload the same lesson; confirm exercises now load fast (already
      persisted) and the completion banner still shows
- [ ] 5.4 Confirm the existing manual "Mark as complete" button still
      works unchanged on a lesson with exercises
- [ ] 5.5 Force an exercises-fetch failure (e.g. stop `ai-exercise-svc`
...
- [ ] 5.6 `npm run lint` and `npm run build` green
- [ ] 5.7 Update `docs/ROADMAP.md` (M3 fully done), `docs/DEVLOG.md`
